...
- Data Segimets in memory
  ...
  - Heap 
    ...
    - grows upward
  - Stack
    - used to store all local varables and used to pass apguments to the functions along with redurn address of the instruction which is to be executed after function is over.
    - grows downward
...
- Scope of Variable
  ...
  - Dynamic scoping - used in Lisp
    ...
    - binding dependes on the flow of control at the run time and the order in which functions are called, refers to the closest active binding.
...
- a point in the execution of a C program where it is sure that, all side effects of previous evalutions are performed, and no side effects from the subsquent evaluations have yet been performed.
